...
contract AMMFactory {
    ...
    // 设置一个默认的 sqrtPriceX96
    uint160 sqrtPriceX96 = 2**96;  // 你可以根据需求调整这个默认值

    // 部署新合约，并传递 token0, token1 和 sqrtPriceX96
    AMMPair newPair = new AMMPair(token0, token1, sqrtPriceX96);
